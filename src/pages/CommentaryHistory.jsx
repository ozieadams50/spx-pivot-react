  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0d1f2d] p-5">
      <div className="mb-2 flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 text-xs text-slate-500">{formatDate(entry.sent_at)}</span>
      </div>
      ...
      <div className="mt-3 flex gap-2">
        {['SPX_Daily', 'SPX_Weekly', 'SPX_Monthly'].map((t) => (
          <span
            key={t}
            className="rounded-full border border-cyan-500/20 bg-cyan-500/10 px-2 py-0.5 text-xs text-cyan-400"
          >
            {t}
          </span>
        ))}
...
export default function CommentaryHistory() {
  ...
  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem('commentary_history') || '[]');
    setEntries(stored);
  }, []);
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Archive of commentary sent to all subscribers — most recent first.
          </p>
        </div>
