...
const LIGHT_BADGE = {
  green:  'bg-emerald-500/15 text-[var(--c-emerald-strong)]',
  yellow: 'bg-amber-500/15 text-[var(--c-amber)]',
  red:    'bg-rose-500/15 text-[var(--c-rose-strong)]',
  blue:   'bg-blue-500/15 text-blue-300',
  grey:   'bg-[var(--c-hover)] text-[var(--c-text-dimmed)]',
};
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-[var(--c-text-primary)]">Log Viewer</h1>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[320px_1fr]">
        <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-3">
          {/* Filter + sort controls */}
          <div className="mb-2 flex items-center gap-0.5 px-1">
            {['all','green','blue','yellow','red','grey'].map(v => (
              <button key={v} onClick={() => setFilterStatus(v)}
                className={`rounded px-1.5 py-0.5 text-xs font-medium transition-colors ${
                  filterStatus === v ? 'bg-cyan-500/25 text-[var(--c-cyan)]' : 'text-[var(--c-text-dimmed)] hover:text-[var(--c-text-secondary)]'
                }`}>
                {v === 'all' ? 'All' : dot(v)}
              </button>
            ))}
            <button onClick={() => setSortByTime(v => !v)} title="Sort by scheduled time"
              className={`ml-auto rounded px-1.5 py-0.5 text-xs transition-colors ${
                sortByTime ? 'text-[var(--c-cyan)]' : 'text-[var(--c-text-dimmed)] hover:text-[var(--c-text-secondary)]'
              }`}>
              ⏰
            </button>
          </div>
          <p className="mb-1 px-2 text-xs font-semibold uppercase tracking-widest text-[var(--c-text-dimmed)]">
            Jobs {visibleJobs.length < CRON_JOBS.length && <span className="normal-case font-normal text-[var(--c-text-faint)]">({visibleJobs.length} of {CRON_JOBS.length})</span>}
          </p>
              ...
              return (
                <button key={j.name} onClick={() => setSelected(j.name)}
                  className={`flex w-full items-center gap-2.5 rounded-xl px-3 py-2.5 text-left text-sm transition-colors ${
                    active ? 'bg-cyan-500/15 text-[var(--c-cyan)]' : 'text-[var(--c-text-muted)] hover:bg-[var(--c-hover)] hover:text-[var(--c-text-secondary)]'
                  }`}>
                  <span className="text-base leading-none">{dot(s.light)}</span>
                  <span className="flex-1 truncate">{j.label}</span>
                  {j.schedule && <span className="text-[10px] text-[var(--c-text-faint)]">{j.schedule.replace('~','').split('-')[0].trim()}</span>}
                  {j.app === 'spx_pivots' && <span className="rounded bg-cyan-500/20 px-1.5 py-0.5 text-[10px] font-semibold text-[var(--c-cyan-strong)]">SPX</span>}
                </button>
              );
            })}
            {visibleJobs.length === 0 && (
              <p className="px-3 py-4 text-xs text-[var(--c-text-faint)]">No jobs match this filter.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-5">
          {loading ? (
            <p className="text-sm text-[var(--c-text-dimmed)]">Loading...</p>
          ) : (
            <>
              <div className="mb-4 flex items-center gap-3">
                <p className="text-base font-semibold text-[var(--c-text-primary)]">{jobDef?.label}</p>
              ...
              <pre className="min-h-[320px] overflow-x-auto rounded-xl border border-[var(--c-border)] bg-black/30 p-4 font-mono text-xs leading-relaxed text-[var(--c-text-secondary)] whitespace-pre-wrap">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-6 border-t border-[var(--c-border)] pt-4">
