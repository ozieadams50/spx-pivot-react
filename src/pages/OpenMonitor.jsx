...
const LIGHT = {
  green:  { border: 'border-emerald-500/30', bg: 'bg-emerald-500/10', text: 'text-[var(--c-emerald-strong)]' },
  yellow: { border: 'border-amber-500/30',   bg: 'bg-amber-500/10',   text: 'text-[var(--c-amber)]'   },
  red:    { border: 'border-rose-500/30',    bg: 'bg-rose-500/10',    text: 'text-[var(--c-rose-strong)]'     },
  grey:   { border: 'border-[var(--c-border)]',       bg: 'bg-[var(--c-hover)]',        text: 'text-[var(--c-text-dimmed)]'    },
};
...
function MetricCard({ label, value, sub }) {
  return (
    <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-4">
      <p className="text-xs font-medium uppercase tracking-widest text-[var(--c-text-dimmed)]">{label}</p>
      <p className="mt-1.5 text-2xl font-bold text-[var(--c-text-primary)]">{value}</p>
      {sub && <p className="mt-0.5 text-xs text-[var(--c-text-dimmed)]">{sub}</p>}
    </div>
  );
}
  ...
  return (
    <div className={`rounded-2xl border p-4 ${s.border} ${s.bg}`}>
      <div className="flex items-start justify-between">
        ...
        {job.app === 'spx_pivots'   && <span className="rounded bg-cyan-500/20   px-1.5 py-0.5 text-[10px] font-semibold text-[var(--c-cyan-strong)]">SPX</span>}
        {job.app === 'pre_earnings' && <span className="rounded bg-violet-500/20 px-1.5 py-0.5 text-[10px] font-semibold text-[var(--c-violet-strong)]">QE</span>}
      </div>
      <p className="mt-2 text-sm font-bold text-[var(--c-text-primary)]">{job.label}</p>
      <p className={`mt-0.5 text-xs font-medium ${s.text}`}>Scheduled: {job.schedule}</p>
      <p className={`mt-2 text-xs ${status.light === 'red' ? 'font-semibold text-[var(--c-rose-strong)]' : s.text}`}>{status.status_text}</p>
    </div>
  );
}
...
function DbCard({ db }) {
  return (
    <div className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-4">
      <p className="font-semibold text-[var(--c-text-primary)]">{db.name}</p>
      <p className="mt-1.5 text-xs text-[var(--c-text-muted)]">Size: <span className="text-[var(--c-text-secondary)]">{db.size_mb?.toFixed(1)} MB</span></p>
      <p className="text-xs text-[var(--c-text-muted)]">Last write: <span className="text-[var(--c-text-secondary)]">{db.modified}</span></p>
      <p className={`mt-2 text-xs font-medium ${db.today ? 'text-[var(--c-emerald-strong)]' : 'text-[var(--c-amber)]'}`}>
...
export default function OpenMonitor() {
  ...
  if (loading || !data) {
    return <div className="flex h-64 items-center justify-center p-8"><p className="text-sm text-[var(--c-text-dimmed)]">Loading system status...</p></div>;
  }
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-[var(--c-text-primary)]">System Monitor</h1>
            {demo && <span className="rounded-lg bg-amber-500/15 px-2 py-0.5 text-xs font-medium text-[var(--c-amber)]">Demo data</span>}
          </div>
          <p className="mt-1 text-sm text-[var(--c-text-muted)]">Last refreshed: {refreshedAt}</p>
        </div>
        <button onClick={load} className="rounded-xl border border-[var(--c-border)] bg-[var(--c-hover)] px-4 py-2 text-sm text-[var(--c-text-secondary)] transition hover:bg-[var(--c-hover-strong)] hover:text-[var(--c-text-primary)]">
          Refresh
        </button>
      </div>
      ...
      <div className="mb-8">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--c-text-dimmed)]">Scheduled Jobs</h2>
      ...
      {data.databases.length > 0 && (
        <div className="mb-8">
          <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--c-text-dimmed)]">Databases</h2>
      ...
      <div className="mb-8">
        <h2 className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--c-text-dimmed)]">Backups</h2>
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="flex items-center rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-4">
            <p className={`text-sm font-semibold ${bkOk ? 'text-[var(--c-emerald-strong)]' : 'text-[var(--c-rose-strong)]'}`}>
              {bkOk ? '🟢 Backups running' : '🔴 No backups found'}
            </p>
          </div>
        </div>
        {data.backups.files?.length > 0 && (
          <div className="mt-3 rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)]">
            <button onClick={() => setBackupsOpen((v) => !v)}
              className="flex w-full items-center justify-between px-4 py-3 text-sm font-medium text-[var(--c-text-secondary)] transition-colors hover:text-[var(--c-text-primary)]">
            ...
            {backupsOpen && (
              <div className="border-t border-[var(--c-border)] px-4 py-3">
                {data.backups.files.map((f) => <p key={f} className="py-0.5 font-mono text-xs text-[var(--c-text-muted)]">{f}</p>)}
              </div>
            )}
          </div>
        )}
