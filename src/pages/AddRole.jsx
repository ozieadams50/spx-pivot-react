  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs font-medium text-[var(--c-text-secondary)]">
        {label}
        {hint && <span className="ml-1.5 font-normal text-[var(--c-text-dimmed)]">{hint}</span>}
      </label>
      {children}
    </div>
  );
...
export default function AddRole() {
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-[var(--c-text-primary)]">Add Role</h1>
        <p className="mt-1 text-sm text-[var(--c-text-muted)]">Define a new custom platform role.</p>
      </div>
      ...
      <div className="max-w-lg rounded-2xl border border-[var(--c-border)] bg-[var(--c-bg-panel)] p-6 shadow-lg">
        <div className="space-y-5">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/admin/roles')} className="flex-1 rounded-xl border border-[var(--c-border)] bg-[var(--c-hover)] px-4 py-2.5 text-sm text-[var(--c-text-secondary)] transition hover:bg-[var(--c-hover-strong)]">
              Cancel
            </button>
          </div>
        </div>
