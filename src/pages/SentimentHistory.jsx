    ...
    <div className="p-6 md:p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">Historical record of market sentiment settings and subscriber alerts.</p>
        </div>
      ...
      {loading ? (
        <div className="space-y-2">{Array.from({length:5}).map((_,i) => <div key={i} className="h-12 animate-pulse rounded-xl bg-white/5"/>)}</div>
      ) : filtered.length === 0 ? (
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((row, i) => (
                <tr key={i} className="transition hover:bg-white/[0.03]">
                  <td className="px-5 py-3 font-mono text-slate-300">{row.setDate}</td>
                  <td className="px-5 py-3">
                    <span className="rounded-lg border border-white/10 bg-white/5 px-2 py-0.5 text-xs capitalize text-slate-300">{row.pivotType}</span>
                  </td>
                  <td className="px-5 py-3"><SentimentBadge sentiment={row.sentiment} /></td>
                  <td className="px-5 py-3 font-mono text-xs text-slate-500">{row.setAt?.slice(0, 16).replace('T', ' ')}</td>
                  <td className="px-5 py-3 text-slate-400">{row.commentary || <span className="text-slate-600">—</span>}</td>
                </tr>
              ))}
            </tbody>
          </table>
